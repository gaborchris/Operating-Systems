...
#define OTP_ENC_FILE "OTP_DEC_SK"
...
void Write_Daemon_Socket(int socketnum){
	int FD;
	char number[16]; //the number to save
...
//Decrypts the text by subtracting the key from each char
//takes in the ciphertext from buffer and the key 
//sets it to 0-26, does the math on it, then sets back to ASCII number
void Encrypt(char buffer[BUFF_SIZE], char key[BUFF_SIZE]){
	...
	for(i=0; i<strlen(buffer); i++){ //loop through entire buffer
		...
		if(buffer[i] == ' '){
			buffer[i] = ((buffer[i] - 6 - tok+27)%27);
		}
		else{
			buffer[i] = ((buffer[i] - 65 - tok+27)%27); //minus 65 then subtract the key, and add 27 to avoid negative nums
		}
		if(buffer[i] == 26){ //26 is space
			buffer[i] += 6;  //make the character go back to its correct char
		}
		else{
			buffer[i] += 65; 
		}
...
//Does the child process that decrypts the message
