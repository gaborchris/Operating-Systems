...
#define OTP_DEC_FILE "OTP_DEC_SK"
...
int main(int argc, char *argv[])
{
	...
	while(eof == false){
		...
		//see if the buffer has reached the end of the line
		if(strstr(readbuffer, "\n") != NULL){
			int termChar = strstr(readbuffer, "\n") - readbuffer;
			//set the last char in the readbuffer to NULL instead of newline
